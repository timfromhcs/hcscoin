...
#include <consensus/quantum.h> // HCScoin: quantum proof validation
...
#include <flatfile.h>
#include <hash.h>
#include <crypto/sha256.h> // HCScoin: quantum fingerprint
...
{
    // Check proof of work matches claimed amount (HCScoin: PoW is computed
    // over the legacy 80-byte header, excluding quantum fields).
    if (fCheckPOW && !CheckProofOfWork(block.GetHashProof(), block.nBits, consensusParams))
...
bool CheckBlock(const CBlock& block, BlockValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // HCScoin: dual consensus - verify the quantum proof for every block
    // except genesis (hashPrevBlock.IsNull()). The header's
    // quantumFingerprint must equal SHA-256 of the full proof, and the
    // proof must reproduce under simulation from the PoW hash.
    if (fCheckPOW && !block.hashPrevBlock.IsNull()) {
        // 1) header commits to full proof
        uint256 fp;
        CSHA256().Write(block.vQuantumProof.data(), block.vQuantumProof.size()).Finalize(fp.begin());
        if (fp != block.quantumFingerprint)
            return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "bad-quantum-fp",
                                 "quantum fingerprint mismatch");
        // 2) quantum difficulty target check
        if (!CheckQuantumDifficulty(block.vQuantumProof, consensusParams.quantumPowLimit))
            return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "high-quantum-hash",
                                 "quantum proof does not meet target");
        // 3) full re-simulation validation
        if (!ValidateQuantumProof(block.vQuantumProof, block.GetHashProof(),
                                  block.nQuantumNonce, consensusParams.nQuantumQubits))
            return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "bad-quantum-proof",
                                 "quantum proof validation failed");
    }
