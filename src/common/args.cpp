...
// HCScoin: configuration file renamed from bitcoin.conf
const char * const BITCOIN_CONF_FILENAME = "hcscoin.conf";
...
fs::path GetDefaultDataDir()
{
    ...
    // HCScoin: renamed from Bitcoin
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "HCScoin";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "HCScoin";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/HCScoin"; // HCScoin
#else
    // Unix-like
    return pathRet / ".hcscoin"; // HCScoin
#endif
#endif
}
