...
struct Params {
    ...
    uint256 powLimit;
    /**
     * HCScoin: Quantum proof-of-work parameters (dual consensus).
     * quantumPowLimit is the target for the quantum proof difficulty hash
     * (see consensus/quantum.h). nQuantumQubits is the circuit width of the
     * Panta-Sim simulation that every block must carry a proof for
     * (27 on public networks; small on regtest for fast tests).
     */
    uint256 quantumPowLimit;
    int nQuantumQubits;
